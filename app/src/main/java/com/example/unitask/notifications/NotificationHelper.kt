...
class NotificationHelper(private val context: Context, private val notificationManager: NotificationManager) {
    fun createChannels() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val reminder = NotificationChannel("reminder", context.getString(com.example.unitask.R.string.notification_channel_reminder_name), NotificationManager.IMPORTANCE_HIGH)
            reminder.description = context.getString(com.example.unitask.R.string.notification_channel_reminder_desc)
            val reward = NotificationChannel("reward", context.getString(com.example.unitask.R.string.notification_channel_reward_name), NotificationManager.IMPORTANCE_DEFAULT)
