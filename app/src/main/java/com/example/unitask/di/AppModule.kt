...
object AppModule {
    ...
    fun configureAppModule(context: Context) {
        ...
        if (_alarmScheduler == null) {
            ...
            val wrapper: AlarmManagerWrapper = RealAlarmManagerWrapper(am)
            _alarmScheduler = AlarmScheduler(wrapper)
        }
        if (_notificationRepository == null) {
            val ctx = _appContext ?: throw IllegalStateException("AppModule context not set")
            val scheduler = _alarmScheduler ?: throw IllegalStateException("AlarmScheduler not initialized")
    ...
    fun configureAppModule(context: Context) {
        ...
        if (_alarmScheduler == null) {
            val am = context.getSystemService(Context.ALARM_SERVICE) as android.app.AlarmManager
                val wrapper: AlarmManagerWrapper = RealAlarmManagerWrapper(am)
                _alarmScheduler = com.example.unitask.notifications.AlarmScheduler(wrapper)
        }
    }
